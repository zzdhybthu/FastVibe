...
<p align="center">
  ...
  <a href="#技术栈"><img src="https://img.shields.io/badge/Code_Agent-6366f1?logo=anthropic&logoColor=white" alt="Code Agent" /></a>
...
<p align="center">
  一个轻量级的 Code Agent 调度中心，支持多仓库管理、任务队列、Git worktree 自动化以及 Web UI 实时交互。
</p>
...
- **用户交互**: Code Agent 可向用户提问，用户在 Web UI 回答
...
FastVibe 采用三级配置体系，各层级职责不同：

### 第一级：配置文件（`config.yaml`）

用于系统级、长期不变或涉及隐私的设置。修改后需重启服务生效。
...
用于个人偏好或较少修改的选项，在设置面板中配置，持久化到浏览器（localStorage）或数据库。

| 设置项 | 存储位置 | 说明 |
|--------|----------|------|
| 仓库管理 | 数据库 | 添加/编辑/删除仓库，设置主分支和并发数 |
| 界面语言 | 浏览器 | 中文 / English |
| 主题 | 浏览器 | 暗色 / 亮色模式 |
...
用于每个任务可能不同的设置，在创建或重启任务时配置。

| 设置项 | 说明 |
|--------|------|
| Agent 类型 | Claude Code / Codex |
| 模型 | 从 config.yaml 中定义的模型列表选择 |
| 思考模式 | 启用更深入的推理 |
| 最大预算 | 单任务预算上限（USD） |
| 交互超时 | 用户确认超时时间（秒） |
...
本项目默认以 dangerous mode 运行 Code Agent，Agent 拥有完整的文件读写和命令执行权限。建议：
...
**Q: 任务刚开始就报错 "Claude Code process exited with code 1"**

A: 请检查终端中 `claude --dangerously-skip-permissions`（Codex 对应 `codex --dangerously-bypass-approvals-and-sandbox`）是否能正常运行。部分环境（如 root 账户）可能会限制这些命令的使用。

**Q: 手机语音输入无响应**

A: 请确认浏览器已开启麦克风权限，并确保网络能正常访问 Google（Chrome 的语音识别依赖 Google 服务）。
