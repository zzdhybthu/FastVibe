...
export default function TaskCard({ task, tinted }: TaskCardProps) {
  ...
  const formatTime = (iso: string): string => {
    ...
    if (diff < 60000) return t.taskCard.justNow;
    if (diff < 3600000) return t.taskCard.minutesAgo(Math.floor(diff / 60000));
    if (diff < 86400000) return t.taskCard.hoursAgo(Math.floor(diff / 3600000));

    return date.toLocaleDateString(language === 'zh' ? 'zh-CN' : 'en-US', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' });
  };
  ...
  const handleCancel = async (e: React.MouseEvent) => {
    e.stopPropagation();
    if (await confirm(t.taskCard.confirmCancel)) {
  ...
  const handleDelete = async (e: React.MouseEvent) => {
    e.stopPropagation();
    if (await confirm(t.taskCard.confirmDelete)) {
  ...
  const statusConfig = getStatusConfig(task.status, language);
