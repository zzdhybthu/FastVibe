...
export async function repoRoutes(app: FastifyInstance) {
  ...
  app.post('/api/repos', async (request, reply) => {
    ...
    if (!existsSync(normalizedPath) || !statSync(normalizedPath).isDirectory()) {
      return reply.code(400).send({ error: `Path does not exist or is not a directory: ${normalizedPath}` });
    }
  ...
  app.put<{ Params: { id: string } }>('/api/repos/:id', async (request, reply) => {
    ...
    // Normalize path if provided and verify it exists
    if (body.path) {
      body.path = normalizePath(body.path);
      if (!existsSync(body.path) || !statSync(body.path).isDirectory()) {
        return reply.code(400).send({ error: `Path does not exist or is not a directory: ${body.path}` });
      }
    }
